...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      {selectedFriend && <FormSplitBill SplitBill={handleSplitBill} selectedFriend={selectedFriend}/>}
    </div>
  )
}
...
function FriendsList ({friends, onSelection, selectedFriend}) {
  return (
    <ul>
      {friends.map((friend) => {
        return <Friend onSelection={onSelection} selectedFriend={selectedFriend} friend={friend}/>
      })}
    </ul>
  )
}
...
function FormSplitBill({selectedFriend, SplitBill}) {
  ...
  return (
    <form className="form-split-bill" onSubmit={handleSubmit}>
      ...
      <label>3{selectedFriend.name} Expense</label>
      <input type="text" value={paidByFriend} disabled/>

      <label>4Who is paying the bills</label>
